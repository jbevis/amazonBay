...
const getInventory = () => {
  ...
  .catch((error) => {
    console.log(error, 'There was an error retrieving the inventory.')
  });
};
...
const processOrder = (total) => {
  ...
  .then(response => {
    if (response.status === 201) {
      ...
      alert('There was an problem in processing your order, please try again.')
    }
  })
  .catch(error => console.log(error, 'There was an error processing your order.'))
};
...
const getOrderHistory = () => {
  ...
  .catch((error) => {
    console.log(error, 'There was an error retrieving order history.')
  });
};
